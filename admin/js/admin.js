...
// ==========================================================================
// 🎨 创作者工坊 (Creative Workshop) 控制器
// ==========================================================================

let allWorkshopPresets = [];
 ...
 * 加载所有工坊预设列表
 */
async function loadWorkshopPresets() {
    ...
    try {
        ...
        if (data.success && Array.isArray(data.presets)) {
            allWorkshopPresets = data.presets;
            if (badge) badge.textContent = `${allWorkshopPresets.length} 个预设`;
 ...
 * 切换工坊分类过滤器
 */
function switchWorkshopFilter(category, buttonEl) {
    currentWorkshopFilter = category;
 ...
 * 根据分类和搜索词过滤预设并渲染
 */
function filterWorkshopPresets() {
    const searchInput = document.getElementById('workshop-search-input');
    const query = (searchInput ? searchInput.value : '').trim().toLowerCase();

    let filtered = allWorkshopPresets.filter(p => {
        // 分类过滤
        if (currentWorkshopFilter !== 'all' && p.category !== currentWorkshopFilter) {
            return false;
        }
        // 搜索关键词过滤 (名称、作者、标签、描述)
        if (query) {
...
function openCreatePresetModal() {
    document.getElementById('preset-modal-title').textContent = '✨ 新建场景预设';
    document.getElementById('preset-name').value = '';
    document.getElementById('preset-category').value = currentWorkshopFilter === 'eavesdrop' ? 'eavesdrop' : 'phone_call';
    document.getElementById('preset-category').disabled = false;
    document.getElementById('preset-id').value = '';
    document.getElementById('preset-id').disabled = false;
    document.getElementById('preset-author').value = 'User';
    document.getElementById('preset-description').value = '';
    document.getElementById('preset-tags').value = '';
    document.getElementById('preset-prompt-template').value = '';
    document.getElementById('preset-temp').value = '0.7';
    document.getElementById('preset-speed').value = '1.0';
    document.getElementById('preset-is-builtin').value = 'false';
 ...
 * 打开编辑预设弹窗
 */
async function openEditPresetModal(category, presetId) {
    try {
        ...
        document.getElementById('preset-modal-title').textContent = p.is_builtin ? `👁️ 查看/基于官方预设另存 (${p.name})` : `✏️ 编辑预设 (${p.name})`;
        document.getElementById('preset-name').value = p.name || '';
        document.getElementById('preset-category').value = p.category || category;
        document.getElementById('preset-category').disabled = true; // 编辑时锁定分类
        document.getElementById('preset-id').value = p.id || presetId;
        document.getElementById('preset-id').disabled = !!p.is_builtin;
        document.getElementById('preset-author').value = p.author || 'User';
        document.getElementById('preset-description').value = p.description || '';
        document.getElementById('preset-tags').value = Array.isArray(p.tags) ? p.tags.join(', ') : (p.tags || '');
        document.getElementById('preset-prompt-template').value = p.prompt_template || '';

        const params = p.recommended_params || {};
        document.getElementById('preset-temp').value = params.temperature !== undefined ? params.temperature : 0.7;
        document.getElementById('preset-speed').value = params.speed !== undefined ? params.speed : 1.0;
        document.getElementById('preset-is-builtin').value = p.is_builtin ? 'true' : 'false';
...
async function savePresetForm() {
    ...
    const rawTags = document.getElementById('preset-tags').value.trim();
    const promptTemplate = document.getElementById('preset-prompt-template').value.trim();
    const isBuiltin = document.getElementById('preset-is-builtin').value === 'true';

    const temp = parseFloat(document.getElementById('preset-temp').value) || 0.7;
    const speed = parseFloat(document.getElementById('preset-speed').value) || 1.0;
 ...
 * 弹出删除确认框
 */
function showDeletePresetDialog(category, presetId, name) {
    pendingDeletePreset = { category, presetId, name };
 ...
 * 执行删除预设
 */
async function executePresetDeletion() {
    if (!pendingDeletePreset) return;
    const { category, presetId, name } = pendingDeletePreset;
 ...
 * 导出预设为 JSON 文件
 */
function exportPreset(category, presetId) {
 ...
 * 触发文件选择框导入预设
 */
function triggerImportPresetFile() {
